...
def _local_feiertage(year, lc):
    ...
    if lc in ("BW", "BY", "ST"):
        hol[datetime.date(year, 1, 6)] = "Hl. Drei Könige"
    if lc in ("BE", "MV"):
        hol[datetime.date(year, 3, 8)] = "Int. Frauentag"
    if lc in ("BW", "BY", "HE", "NW", "RP", "SL"):
        hol[e + datetime.timedelta(60)] = "Fronleichnam"
    if lc in ("BY", "SL"):
        hol[datetime.date(year, 8, 15)] = "Mariä Himmelfahrt"
    if lc == "TH":
        hol[datetime.date(year, 9, 20)] = "Weltkindertag"
    if lc in ("BB", "HB", "HH", "MV", "NI", "SN", "ST", "SH", "TH"):
        hol[datetime.date(year, 10, 31)] = "Reformationstag"
    if lc in ("BW", "BY", "NW", "RP", "SL"):
        hol[datetime.date(year, 11, 1)] = "Allerheiligen"
    if lc == "SN":
...
def fetch_feiertage(year, bundesland, verbose=True):
    lc = LAND_CODES.get(bundesland.lower())
    if not lc:
        print(f"Unbekanntes Bundesland: {bundesland}")
        sys.exit(1)
...
def fetch_year_feiertage(yc: YearConfig, verbose=True, fetcher=fetch_feiertage):
    """Fetch holidays for every configured period, allowing state changes mid-year."""
    cache = {}
    combined = {}

    for period in yc.periods:
        if period.land_code not in cache:
            cache[period.land_code] = fetcher(
                yc.year, period.bundesland, verbose=verbose
            )

        for holiday_date, holiday_name in cache[period.land_code].items():
            if period.start_date <= holiday_date <= period.end_date:
                combined[holiday_date] = holiday_name
...
def classify(year, feiertage, yc: YearConfig):
    ...
    if yc.has_explicit_urlaub:
        r = _apply_explicit_urlaub(year, r, yc.explicit_urlaub_dates, yc.total_urlaub)
...
def _apply_explicit_urlaub(year, r, vacation_dates, urlaub_count):
    allocated = 0
    ignored = 0
    for d in sorted(vacation_dates):
        if d.year != year or d not in r:
            ignored += 1
            continue
        if _is_workday(d, r):
            r[d] = (T.U, "Dokumentierter Urlaub")
            allocated += 1
        else:
            ignored += 1

    r["_urlaub_mode"] = "explicit"
    r["_urlaub_requested"] = urlaub_count
    r["_urlaub_remaining"] = max(0, urlaub_count - allocated)
    r["_urlaub_allocated"] = allocated
    r["_urlaub_ignored"] = ignored
    r["_urlaub_excess"] = max(0, allocated - urlaub_count)
    return r
...
def astats(year, dm):
    ...
    while d <= datetime.date(year, 12, 31):
        dt = dm[d][0]
        s[dt] = s.get(dt, 0) + 1
        if dt == T.F and d.weekday() < 5:
            s["fw"] += 1
        d += datetime.timedelta(1)
    s["u_remaining"] = dm.get("_urlaub_remaining", 0)
    s["u_allocated"] = dm.get("_urlaub_allocated", 0)
    s["u_requested"] = dm.get("_urlaub_requested", 0)
    s["u_mode"] = dm.get("_urlaub_mode", "none")
    s["u_ignored"] = dm.get("_urlaub_ignored", 0)
    s["u_excess"] = dm.get("_urlaub_excess", 0)
    return s
...
def _state_subtitle(yc: YearConfig):
    seen = []
    for period in yc.periods:
        label = f"{period.bundesland} ({period.land_code})"
        if label not in seen:
            seen.append(label)
    return " | ".join(seen) if seen else "Unbekannt"


def build_year_summary(yc: YearConfig, ann, wk, output_path: str | None = None):
    return {
        "year": yc.year,
        "states": [
            {
                "label": period.label,
                "bundesland": period.bundesland,
                "land_code": period.land_code,
            }
            for period in yc.periods
        ],
        "urlaub": {
            "mode": ann.get("u_mode", "none"),
            "requested": ann.get("u_requested", yc.total_urlaub),
            "allocated": ann.get("u_allocated", 0),
            "remaining": ann.get("u_remaining", 0),
            "ignored": ann.get("u_ignored", 0),
            "excess": ann.get("u_excess", 0),
        },
        "workdays": {
            "office": ann[T.B],
            "home_office": ann[T.H],
            "holidays": ann[T.F],
            "holidays_on_workdays": ann["fw"],
            "weekends": ann[T.W],
            "vacation": ann[T.U],
        },
        "werbungskosten": wk.to_dict(),
        "output_pdf": output_path,
    }


def default_output_filename(yc: YearConfig):
    suffix = yc.bundesland if not yc.uses_multiple_states else "Mehrere_Bundeslaender"
    return f"Arbeitstage_{yc.year}_{suffix}.pdf"
...
def generate(metadata_path, output_path=None):
    ...
    for yc in year_configs:
        out = output_path or str(Path(base_dir) / default_output_filename(yc))
        summaries.append(generate_year(yc, personal, base_dir, out))
    return summaries
...
def generate_year(
    ...
    base_dir: str,
    output_path: str,
    include_anlage_n: bool = False,
    verbose: bool = False,
):
    """Generate PDF for one year."""
    year = yc.year
    subtitle = _state_subtitle(yc)
    print(f"Generiere {year}: {subtitle}...")

    feiertage = fetch_year_feiertage(yc, verbose=verbose)
    ...
    from tax_summary import compute_werbungskosten

    wk = compute_werbungskosten(year, yc, dm)

    _build_pdf(
        year, yc, personal, dm, feiertage, ann, wk, output_path, include_anlage_n
    )
    print(f"  PDF erstellt: {output_path}")
    return build_year_summary(yc, ann, wk, output_path)
...
def _build_pdf(
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=10 * mm,
        bottomMargin=10 * mm,
        title=f"Arbeitstage {year}",
        author="Tax-Automate-Germany",
    )
    pw = landscape(A4)[0] - 20 * mm

    el = []
    state_subtitle = _state_subtitle(yc)
    ...
    _add_title_bar(
        el,
        f"ARBEITSTAGE-KALENDER {year}",
        state_subtitle + (f"  |  {name}" if name else ""),
        pw,
    )
    el.append(Spacer(1, 3 * mm))
    ...
    for start in (1, 7):
        _add_title_bar(
            el,
            f"MONATSKALENDER {year}",
            f"{MON3[start]}-{MON3[min(start+5,12)]}  |  {state_subtitle}",
            pw,
        )
        el.append(Spacer(1, 2 * mm))
        el.append(
            _month_grids(
                year, dm, feiertage, list(range(start, min(start + 6, 13))), pw
            )
        )
        el.append(Spacer(1, 2 * mm))
        el.append(_legend(pw))
    ...
    # ========== PAGE 4: Summary ==========
    _add_title_bar(el, f"JAHRESÜBERSICHT {year}", state_subtitle, pw)
    el.append(Spacer(1, 3 * mm))
...
def _cover_col1(personal: PersonalData, yc: YearConfig, w):
    ...
    rows.append(
        [
            Paragraph(
                "<b>Persönliche Daten</b>", _ps("c1h", 8, True, C_WHITE, TA_LEFT)
            ),
            "",
        ]
    )
    if personal.full_name:
        rows.append(
            [
                Paragraph("Name", _ps("c1_n", 7, color=C_MID, align=TA_LEFT)),
                Paragraph(
                    f"<b>{personal.full_name}</b>",
                    _ps("c1_nv", 7, True, C_DARK, TA_LEFT),
                ),
            ]
        )
    if personal.geburtsdatum:
        rows.append(
            [
                Paragraph("Geb.datum", _ps("c1_g", 7, color=C_MID, align=TA_LEFT)),
                Paragraph(
                    personal.geburtsdatum, _ps("c1_gv", 7, color=C_DARK, align=TA_LEFT)
                ),
            ]
        )
    if personal.steuer_id:
        rows.append(
            [
                Paragraph("Steuer-ID", _ps("c1_s", 7, color=C_MID, align=TA_LEFT)),
                Paragraph(
                    personal.steuer_id, _ps("c1_sv", 7, color=C_DARK, align=TA_LEFT)
                ),
            ]
        )
    if personal.finanzamt:
        rows.append(
            [
                Paragraph("Finanzamt", _ps("c1_f", 7, color=C_MID, align=TA_LEFT)),
                Paragraph(
                    personal.finanzamt, _ps("c1_fv", 7, color=C_DARK, align=TA_LEFT)
                ),
            ]
        )
    if personal.steuerklasse:
        rows.append(
            [
                Paragraph("Steuerklasse", _ps("c1_k", 7, color=C_MID, align=TA_LEFT)),
                Paragraph(
                    personal.steuerklasse, _ps("c1_kv", 7, color=C_DARK, align=TA_LEFT)
                ),
            ]
        )
    ...
    for pi, p in enumerate(yc.periods):
        label = (
            f"Arbeitskonfiguration"
            if len(yc.periods) == 1
            else f"Konfiguration {p.label}"
        )
        rows.append(
            [
                Paragraph(
                    f"<b>{label}</b>", _ps(f"c1_ch{pi}", 8, True, C_WHITE, TA_LEFT)
                ),
                "",
            ]
        )
        rows.append(
            [
                Paragraph(
                    "Arbeitstage", _ps(f"c1_at{pi}", 7, color=C_MID, align=TA_LEFT)
                ),
                Paragraph(
                    f"<b>{p.work_label}</b>",
                    _ps(f"c1_atv{pi}", 7, True, C_DARK, TA_LEFT),
                ),
            ]
        )
        rows.append(
            [
                Paragraph(
                    "Home-Office", _ps(f"c1_ho{pi}", 7, color=C_MID, align=TA_LEFT)
                ),
                Paragraph(
                    f"<b>{p.ho_label}</b>",
                    _ps(f"c1_hov{pi}", 7, True, C_ACCENT, TA_LEFT),
                ),
            ]
        )
        rows.append(
            [
                Paragraph(
                    "Entfernung", _ps(f"c1_km{pi}", 7, color=C_MID, align=TA_LEFT)
                ),
                Paragraph(
                    f"<b>{p.km:.0f} km</b>",
                    _ps(f"c1_kmv{pi}", 7, True, C_DARK, TA_LEFT),
                ),
            ]
        )
    ...
    rows.append(
        [
            Paragraph("Urlaubstage", _ps("c1_u", 7, color=C_MID, align=TA_LEFT)),
            Paragraph(
                f"<b>{yc.total_urlaub} Tage</b>",
                _ps("c1_uv", 7, True, C_GREEN, TA_LEFT),
            ),
        ]
    )
    if yc.has_explicit_urlaub:
        rows.append(
            [
                Paragraph("Dokumentiert", _ps("c1_ud", 7, color=C_MID, align=TA_LEFT)),
                Paragraph(
                    f"<b>{len(yc.explicit_urlaub_dates)} Tage</b>",
                    _ps("c1_udv", 7, True, C_ACCENT, TA_LEFT),
                ),
            ]
        )
...
def _cover_col2(ann, yc: YearConfig, wk, w, year):
    ...
    ho = ann[T.H]
    arbeit = buero + ho
    explicit_urlaub = ann.get("u_mode") == "explicit"
    urlaub_label = "Urlaub dokumentiert" if explicit_urlaub else "Urlaubstage geplant"
    remaining_label = "noch offen" if explicit_urlaub else "davon frei planbar"

    rows = [
        [
            Paragraph(
                "<b>Steuerrelevante Kennzahlen</b>",
                _ps("c2h", 8, True, C_WHITE, TA_LEFT),
            ),
            "",
        ],
        [
        ...
        [
            Paragraph(
                "Arbeitstage gesamt", _ps("c2_at", 7, color=C_MID, align=TA_LEFT)
            ),
            Paragraph(f"<b>{arbeit}</b>", _ps("c2_atv", 11, True, C_DARK, TA_RIGHT)),
        ],
        [
            Paragraph(
                "Feiertage (Werktag)", _ps("c2_ft", 7, color=C_MID, align=TA_LEFT)
            ),
            Paragraph(
                f"<b>{ann['fw']}</b>", _ps("c2_ftv", 11, True, C_ORANGE, TA_RIGHT)
            ),
        ],
        [
            Paragraph(urlaub_label, _ps("c2_ur", 7, color=C_MID, align=TA_LEFT)),
            Paragraph(
                f"<b>{ann.get(T.U, 0)}</b> / {yc.total_urlaub}",
                _ps("c2_urv", 9, True, C_GREEN, TA_RIGHT),
            ),
        ],
        [
            Paragraph(remaining_label, _ps("c2_uf", 7, color=C_MID, align=TA_LEFT)),
            Paragraph(
                f"<b>{ann.get('u_remaining', 0)}</b>",
                _ps("c2_ufv", 9, True, C_MID, TA_RIGHT),
            ),
        ],
    ]
    if explicit_urlaub and ann.get("u_ignored", 0):
        rows.append(
            [
                Paragraph(
                    "nicht angerechnet", _ps("c2_ui", 7, color=C_MID, align=TA_LEFT)
                ),
                Paragraph(
                    f"<b>{ann['u_ignored']}</b>",
                    _ps("c2_uiv", 9, True, C_ORANGE, TA_RIGHT),
                ),
            ]
        )
    ...
    rows.append(
        [
            Paragraph(
                "<b>Fahrtkostenberechnung</b>", _ps("c2_fh", 8, True, C_WHITE, TA_LEFT)
            ),
            "",
        ]
    )
    ...
    if len(wk.periods) > 1:
        for i, pr in enumerate(wk.periods):
            rows.append(
                [
                    Paragraph(
                        f"{pr.label} ({pr.km:.0f} km)",
                        _ps(f"c2_fp{i}", 6.5, color=C_MID, align=TA_LEFT),
                    ),
                    Paragraph(
                        f"<b>{_fmt_eur(pr.ep)}</b>",
                        _ps(f"c2_fpv{i}", 7, True, C_DARK, TA_RIGHT),
                    ),
                ]
            )
    else:
        ...
        rows.append(
            [
                Paragraph(
                    "Einfache Entfernung", _ps("c2_km", 7, color=C_MID, align=TA_LEFT)
                ),
                Paragraph(
                    f"<b>{pr.km:.0f} km</b>", _ps("c2_kmv", 8, True, C_DARK, TA_RIGHT)
                ),
            ]
        )
        rows.append(
            [
                Paragraph(
                    "Fahrten (= Bürotage)", _ps("c2_fb", 7, color=C_MID, align=TA_LEFT)
                ),
    ...
    rows.append(
        [
            Paragraph(
                "Entfernungspauschale", _ps("c2_ep", 7, color=C_MID, align=TA_LEFT)
            ),
            Paragraph(
                f"<b>{_fmt_eur(wk.total_ep)}</b>",
                _ps("c2_epv", 9, True, C_GREEN, TA_RIGHT),
            ),
        ]
    )
    rows.append(
        [
            Paragraph(
                "Homeoffice-Pauschale", _ps("c2_hp", 7, color=C_MID, align=TA_LEFT)
            ),
            Paragraph(
                f"<b>{_fmt_eur(wk.ho_pauschale)}</b>",
                _ps("c2_hpv", 9, True, C_ACCENT, TA_RIGHT),
            ),
        ]
    )
    rows.append(
        [
            Paragraph(
                "Weitere Werbungskosten", _ps("c2_we", 7, color=C_MID, align=TA_LEFT)
            ),
            Paragraph(
                f"<b>{_fmt_eur(wk.additional_expenses_total)}</b>",
                _ps("c2_wev", 9, True, C_DARK, TA_RIGHT),
            ),
        ]
    )
    rows.append(
        [
            Paragraph(
                "<b>Werbungskosten ges.</b>",
                _ps("c2_wk", 7, True, C_MID, align=TA_LEFT),
            ),
            Paragraph(
    ...
    rows.append(
        [
            Paragraph(
                "ueber Pauschbetrag", _ps("c2_pb", 7, color=C_MID, align=TA_LEFT)
            ),
            Paragraph(
                f"<b>{_fmt_eur(wk.net_benefit)}</b>" if wk.above_pauschbetrag else "-",
                _ps(
                    "c2_pbv",
                    9,
                    True,
                    C_GREEN if wk.above_pauschbetrag else C_MID,
                    TA_RIGHT,
                ),
            ),
        ]
...
def _cover_col3(feiertage, yc: YearConfig, w):
    ...
    header_right = "periodabhaengig" if yc.uses_multiple_states else yc.bundesland
    rows = [
        [
            Paragraph("<b>Feiertage</b>", _ps("c3h", 8, True, C_WHITE, TA_LEFT)),
            Paragraph(
                f"<b>{header_right}</b>", _ps("c3h2", 7, True, C_BLUE_L, TA_RIGHT)
            ),
        ],
    ]
    for d in sorted(feiertage.keys()):
        on_wd = d.weekday() < 5
        period = yc.period_for_date(d)
        state_suffix = f" ({period.land_code})" if yc.uses_multiple_states else ""
        rows.append(
            [
                Paragraph(
                    f"<b>{d.strftime('%d.%m.')}</b> {WT_KURZ[d.weekday()]}",
                    _ps(
                        f"c3d{d.toordinal()}",
                        6.5,
                        color=C_DARK if on_wd else C_MID,
                        align=TA_LEFT,
                    ),
                ),
                Paragraph(
                    f"{feiertage[d]}{state_suffix}" + ("" if on_wd else " <i>(WE)</i>"),
                    _ps(
                        f"c3n{d.toordinal()}",
                        6.5,
                        color=C_RED_D if on_wd else C_MID,
                        align=TA_LEFT,
                    ),
                ),
            ]
    ...
    for pi, p in enumerate(yc.periods):
        ...
        rows.append(
            [
                Paragraph(
                    f"<b>{label}</b>", _ps(f"c3ah{pi}", 8, True, C_WHITE, TA_LEFT)
                ),
                "",
            ]
        )
        if p.addr_home:
            rows.append(
                [
                    Paragraph(
                        "Wohnung", _ps(f"c3w{pi}", 6.5, color=C_MID, align=TA_LEFT)
                    ),
                    Paragraph(
                        p.addr_home, _ps(f"c3wv{pi}", 6.5, color=C_DARK, align=TA_LEFT)
                    ),
                ]
            )
        if p.addr_work:
            rows.append(
                [
                    Paragraph(
                        "Arbeit", _ps(f"c3a{pi}", 6.5, color=C_MID, align=TA_LEFT)
                    ),
                    Paragraph(
                        p.addr_work, _ps(f"c3av{pi}", 6.5, color=C_DARK, align=TA_LEFT)
                    ),
                ]
            )
    ...
    for i, row in enumerate(rows):
        if isinstance(row[1], str) and row[1] == "":
            ...
            style.append(
                ("BACKGROUND", (0, i), (-1, i), C_FEIER if i % 2 == 0 else C_FEIER_ALT)
            )
        elif i % 2 == 0:
            style.append(("BACKGROUND", (0, i), (-1, i), C_FAINT))
    tbl.setStyle(TableStyle(style))
...
def _add_map_section(el, yc: YearConfig, pw):
    ...
    if len(maps) == 1:
        ...
        comment_p = (
            Paragraph(
                f'<i>"{comment}"</i>',
                _ps("mrc", 7, color=C_MID, align=TA_LEFT, lead=10),
            )
            if comment
            else ""
        )
        ...
        for i, (mf, comment, label) in enumerate(maps):
            header.append(
                Paragraph(
                    f"<b>Route: {label}</b>", _ps(f"mrh{i}", 8, True, C_WHITE, TA_LEFT)
                )
            )
...
def _legend(pw):
    ...
    cells = [
        Paragraph(lb, _ps(f"lg_{i}", 7, color=C_DARK))
        for i, (_, lb) in enumerate(items)
    ]
    cw = pw / len(items)
...
def _one_month(year, month, dm, feiertage, bw):
    ...
    summary = f"B:{ms[T.B]} H:{ms[T.H]} F:{ms[T.F]}" + (
        f" U:{u_count}" if u_count else ""
    )

    hdr = (
        [
            Paragraph(
                f"<b>{MONATE[month]} {year}</b>", _ps("mh", 8, True, C_WHITE, TA_LEFT)
            )
        ]
        + [""] * 6
        + [Paragraph(summary, _ps("ms", 5.5, color=C_BLUE_L, align=TA_RIGHT))]
    )
    ...
    for wi, week in enumerate(cal_obj.monthdayscalendar(year, month)):
        ...
        for dn in week:
            if dn > 0:
                kw = datetime.date(year, month, dn).isocalendar()[1]
                break
        ...
        for di, dn in enumerate(week):
            if dn == 0:
                ...
                if dt == T.F:
                    row.append(
                        Paragraph(f"<b>{dn}</b>", _ps(f"c_{dn}", 6.5, True, C_RED_D))
                    )
                elif dt == T.U:
                    row.append(
                        Paragraph(f"<b>{dn}</b>", _ps(f"c_{dn}", 6.5, True, C_GREEN))
                    )
                elif dt == T.W:
                    row.append(Paragraph(str(dn), _ps(f"c_{dn}", 6, color=C_MID)))
                elif dt == T.H:
                    row.append(
                        Paragraph(f"<b>{dn}</b>", _ps(f"c_{dn}", 6.5, True, C_ACCENT))
                    )
                else:
    ...
    ft = [
        Paragraph(
            f"<b>B</b>:{ms[T.B]}  <b>H</b>:{ms[T.H]}  <b>F</b>:{ms[T.F]}  <b>W</b>:{ms[T.W]}{u_str}",
            _ps("mf", 5.5, color=C_MID, align=TA_LEFT),
        )
    ] + [""] * 7
    data.append(ft)
    fi = len(data) - 1
...
def _summary_table(year, dm, yc: YearConfig, pw):
    ...
    for m in range(1, 13):
        ...
        fw = sum(
            ...
            if dm[datetime.date(year, m, d2)][0] == T.F
        ...
        row = [
            Paragraph(
                f"<b>{MONATE[m]}</b>", _ps(f"sm_{m}", 6.5, True, C_DARK, TA_LEFT)
            ),
            Paragraph(str(ms2[T.B]), vs),
            Paragraph(str(ms2[T.H]), vs),
            Paragraph(
                f"<b>{mu}</b>" if mu else "-",
                _ps(f"su_{m}", 6.5, bold=mu > 0, color=C_GREEN if mu else C_MID),
            ),
            Paragraph(f"<b>{at}</b>", _ps(f"sa_{m}", 6.5, True)),
            Paragraph(str(ms2[T.F]), vs),
            Paragraph(str(fw), vs),
            Paragraph(str(ms2[T.W]), vs),
            Paragraph(str(nd), vs),
            Paragraph(f"{bp}%", vs),
            Paragraph(f"{hp}%", vs),
            Paragraph(kws, _ps(f"skw_{m}", 6, color=C_MID)),
            Paragraph(
                f"{m_km:,.0f}".replace(",", "."), _ps(f"skm_{m}", 6.5, color=C_ACCENT)
            ),
        ]
        data.append(row)
        ri = len(data) - 1
...
if __name__ == "__main__":
    ...
    for yc in year_configs:
        yout = out or str(Path(base_dir) / default_output_filename(yc))
